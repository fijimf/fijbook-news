...
final case class RssFeedUpdateImpl[M[_]](httpClient: Client[M], repo: RssRepo[M])(implicit z: Async[M])  extends RssFeedUpdate [M]{
  val log:Logger=LoggerFactory.getLogger(getClass)

  private def getUpdates(feedId: Long): M[List[RssItem]] = {
    def infoSafely(msg: String): OptionT[M, Unit] = OptionT.liftF(z.delay(log.info(msg)))
    def debugSafely(msg: String): OptionT[M, Unit] = OptionT.liftF(z.delay(log.info(msg)))
    ...
    (for {
      ...
      createdItems <- OptionT.fromOption[M](createItems(xml, feed.id))
      _ <- infoSafely(s"Created ${createdItems.length} items.")
      _ <- infoSafely("Items:\n\t"+createdItems.map(_.title.take(36)+"...").mkString(",\n\t"))
    } yield {
      createdItems
    }).getOrElseF(z.pure(List.empty[RssItem]))
  }
  ...
  override def updateFeed(feedId: Long): M[fs2.Stream[M,RssItem]] = {
    for{
      items<-getUpdates(feedId)
      _ <- z.delay(log.info(s"Retrieved ${items.size} items from the feed"))
